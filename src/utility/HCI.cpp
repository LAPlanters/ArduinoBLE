...
String metaEventToString(LE_META_EVENT event)
{
  switch(event){
    ...
    case PHY_UPDATE_COMPLETE: return F("PHY_UPDATE_COMPLETE");
...
void HCIClass::handleEventPkt(uint8_t /*plen*/, uint8_t pdata[])
{
  ...
  else if (eventHdr->evt == EVT_LE_META_EVENT)
  {
    ...
    switch((LE_META_EVENT)leMetaHeader->subevent){
      ...
      case PHY_UPDATE_COMPLETE:{
        struct __attribute__ ((packed)) EvtPhyUpdateComplete {
          uint8_t  status;
          uint16_t handle;
          uint8_t  txPhy;   // 1=1M, 2=2M, 3=Coded
          uint8_t  rxPhy;
        } *phyUpdate = (EvtPhyUpdateComplete*)&pdata[sizeof(HCIEventHdr) + sizeof(LeMetaEventHeader)];
#ifdef _BLE_TRACE_
        Serial.print("[BLE] PHY Update Complete: status=");
        Serial.print(phyUpdate->status, HEX);
        Serial.print(" handle=");
        Serial.print(phyUpdate->handle);
        Serial.print(" TX=");
        Serial.print(phyUpdate->txPhy);
        Serial.print(" RX=");
        Serial.println(phyUpdate->rxPhy);
#endif
        (void)phyUpdate; // avoid unused warning when trace is off
        break;
      }
      case CONN_COMPLETE:{
        struct __attribute__ ((packed)) EvtLeConnectionComplete {
...
int HCIClass::leSetDefaultPhy(uint8_t allPhys, uint8_t txPhys, uint8_t rxPhys) {
  struct __attribute__((packed)) {
    uint8_t allPhys;
    uint8_t txPhys;
    uint8_t rxPhys;
  } params = { allPhys, txPhys, rxPhys };
  return sendCommand(OGF_LE_CTL << 10 | LE_COMMAND::SET_DEFAULT_PHY, sizeof(params), &params);
}

int HCIClass::leSetPhy(uint16_t connectionHandle, uint8_t allPhys,
  ...
  struct __attribute__((packed)) {
    ...
    uint8_t rxPhys;
    uint16_t phyOptions;
  } params = { connectionHandle, allPhys, txPhys, rxPhys, phyOptions };
  // Note: HCI_LE_Set_PHY is a command that generates a Command Status event
  // (not Command Complete), followed by an LE PHY Update Complete event.
  return sendCommand(OGF_LE_CTL << 10 | LE_COMMAND::SET_PHY, sizeof(params), &params);
}

int HCIClass::leReadPhy(uint16_t connectionHandle, uint8_t& txPhy, uint8_t& rxPhy) {
  int result = sendCommand(OGF_LE_CTL << 10 | LE_COMMAND::READ_PHY, 2, &connectionHandle);
  if (result == 0) {
    // Command Complete response: handle(2) + txPhy(1) + rxPhy(1)
    txPhy = _cmdResponse[2];
    rxPhy = _cmdResponse[3];
  }
  return result;
}
