...
public:
  ...
  // BLE 5.0 PHY control
  virtual int leReadPhy(uint16_t connectionHandle, uint8_t& txPhy, uint8_t& rxPhy);
  virtual int leSetDefaultPhy(uint8_t allPhys, uint8_t txPhys, uint8_t rxPhys);
  virtual int leSetPhy(uint16_t connectionHandle, uint8_t allPhys, uint8_t txPhys, uint8_t rxPhys, uint16_t phyOptions);
