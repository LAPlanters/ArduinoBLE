...
public:
  ...
  // BLE 5.0 PHY control
  // phys: bit 0 = 1M, bit 1 = 2M, bit 2 = Coded (OR together for preferences)
  virtual int setPreferredPhy(uint8_t txPhys, uint8_t rxPhys);
  virtual int requestPhy(uint16_t connectionHandle, uint8_t txPhys, uint8_t rxPhys, uint16_t phyOptions = 0x0000);
  virtual int readPhy(uint16_t connectionHandle, uint8_t& txPhy, uint8_t& rxPhy);
