...
int BLELocalDevice::setPreferredPhy(uint8_t txPhys, uint8_t rxPhys)
{
  // allPhys=0 means "use the txPhys/rxPhys preferences" (no "no preference" bits set)
  return HCI.leSetDefaultPhy(0x00, txPhys, rxPhys);
}

int BLELocalDevice::requestPhy(uint16_t connectionHandle, uint8_t txPhys, uint8_t rxPhys, uint16_t phyOptions)
{
  return HCI.leSetPhy(connectionHandle, 0x00, txPhys, rxPhys, phyOptions);
}

int BLELocalDevice::readPhy(uint16_t connectionHandle, uint8_t& txPhy, uint8_t& rxPhy)
{
  return HCI.leReadPhy(connectionHandle, txPhy, rxPhy);
}
